...
export async function fetchYearSeries(yearId) {
  ...
  if (cardIds.length) {
    const { data } = await supabase
      .from('card_expenses')
      .select('card_id, monto_total, monto_total_ars, compartido_con, pct_compartido')
      .in('card_id', cardIds);
    cardExp = data || [];
  }
  ...
  return (months || []).map((m) => {
    ...
    const cardSum = cardExp
      ...
      .reduce((s, e) => {
        ...
        return s + base * (1 - pct / 100); // tu parte en pesos
      }, 0);
...
export async function propagateCardInstallments(cardId, payload) {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    await supabase.from('card_expenses').insert({
      ...
      pct_compartido: payload.pct_compartido ?? 50,
    });
  }
}
